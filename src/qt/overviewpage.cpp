...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizePhoreAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizePhoreAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizePhoreAmount = strAnonymizePhoreAmount.remove(strAnonymizePhoreAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePhoreAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizePhoreAmount * COIN) nMaxToAnonymize = nAnonymizePhoreAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizePhoreAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizePhoreAmount));
        strAnonymizePhoreAmount = strAnonymizePhoreAmount.remove(strAnonymizePhoreAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePhoreAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
